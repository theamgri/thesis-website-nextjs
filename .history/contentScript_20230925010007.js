...
const facebookChatInput = document.querySelector('div[contenteditable="true"]'); // Adjust the selector as needed
...
if (facebookChatInput) {
  ...
  facebookChatInput.addEventListener('keydown', function(event) {
    if (event.key === 'Enter') {
      ...
      // Optionally, you can also send the chat input if there's a "Send" button by simulating a click
      const sendButton = document.querySelector('button[type="submit"]'); // Adjust the selector as needed
      if (sendButton) {
        sendButton.click();
      }
    }
