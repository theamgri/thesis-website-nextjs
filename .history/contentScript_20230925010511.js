...
if (instagramChatInput && sendButton) {
  ...
  console.error("Instagram chat input or send button not found.");
}
...
const submitButton = document.querySelector('div.x1ey2m1c.xds687c.xg01cxk.x47corl.x10l6tqk.x17qophe.x13vifvy.x1ebt8du.x19991ni.x1dhq9h.x1wpzbip.x14yjl9h.xudhj91.x18nykt9.xww2gxu[data-visualcompletion="ignore"]');

if (facebookChatInput && submitButton) {
  // Add an event listener to the input field to capture input
  facebookChatInput.addEventListener('input', function() {
    // Get the input value from the text content
  ...
  facebookChatInput.addEventListener('keydown', function(event) {
    if (event.key === 'Enter') {
      ...
      // Optionally, trigger the send action here (click the submit button)
      submitButton.click();
    }
  });
} else {
  console.error("Facebook chat input or submit button not found.");
}
